...
> **版本**：v2.4  
> **主要更新**：新增自动舍友推荐专项测试数据脚本，覆盖兼容室友、不兼容室友和无问卷室友场景  
...
`004_seed_roommate_recommendation_test_data.sql` 用于验证新生分配中的自动舍友/床位推荐逻辑。脚本创建独立的 `自动推荐测试楼`：

- 701：`student102`、`student103` 已入住，问卷与待分配新生 `student101` 高度兼容。
- 702：`student104`、`student105` 已入住，吸烟、打鼾、作息和学习习惯与 `student101` 明显不匹配。
- 703：`student106` 已入住但没有问卷，用于验证无画像舍友只获得低权重基础分。

脚本会重置自身测试房间床位、测试用户问卷和相关分配请求。执行后以 `student101` 调用 `POST /api/allocations`，预期推荐 `自动推荐测试楼 701 C床`。
