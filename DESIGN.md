...
数据库脚本 `sql/004_seed_roommate_recommendation_test_data.sql` 提供了可重复执行的推荐算法专项测试场景：`student101` 为待分配新生，701 房间包含两名生活习惯高度兼容的已入住学生，702 房间包含明显不兼容学生，703 房间包含无问卷学生。调用 `POST /api/allocations` 后，预期推荐 `自动推荐测试楼 701 C床`。
